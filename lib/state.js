...
export function r2SetTopicById(topicId) {
  ...
  if (!Number.isFinite(topicId) || topicId <= 0) {
    ...
    state.roundTwo.manualCorrectUserIds = [];
...
export function r2SetManualCorrectUsers(userIds) {
  const state = getGameState();
  const teams = getTeams();
  ...
  const cleaned = Array.from(
    new Set(
      list
        .map((id) => Number(id))
        .filter((id) => Number.isFinite(id) && teamIds.has(id))
    )
  );
  state.roundTwo.manualCorrectUserIds = cleaned;
  saveGameState(state);
  broadcast(makeEvent("r2:manual-correct", { userIds: cleaned }));
  return { ok: true, state };
}
...
export function r2Settle() {
  const state = getGameState();
  const manualList = Array.isArray(state.roundTwo.manualCorrectUserIds)
    ? state.roundTwo.manualCorrectUserIds
    : [];
  ...
  const useManual = manualSet.size > 0;
  const correctStr = String(state.roundTwo.correctAnswer ?? "")
    .trim()
    .toLowerCase();
  if (!useManual && !correctStr) {
    return { ok: false, error: "Correct answer not set" };
  }
  ...
  const correctTeams = useManual
    ? bettors.filter((u) => manualSet.has(String(u)))
    : bettors.filter(
        (u) =>
          String(answers[u] ?? "")
            .trim()
            .toLowerCase() === correctStr
      );
